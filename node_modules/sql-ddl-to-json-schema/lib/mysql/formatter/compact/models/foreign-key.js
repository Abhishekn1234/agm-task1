...
class ForeignKey {
    ...
    static fromDef(json) {
        if (json.id === 'O_CREATE_TABLE_CREATE_DEFINITION') {
            if ((0, utils_1.isDefined)(json.def.foreignKey)) {
    ...
    hasAllColumnsFromRefTable(table) {
        return ((table.columns ?? []).filter((tableColumn) => this.reference.columns?.some((indexColumn) => indexColumn.column === tableColumn.name)).length === this.reference.columns?.length);
    }
    /**
